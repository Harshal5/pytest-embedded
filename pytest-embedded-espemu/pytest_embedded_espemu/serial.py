...
from .espemu import send_control_command
...
class EspEmuSerial:
    ...
    def __init__(self, control_port: int | None = None, app: t.Optional['IdfApp'] = None) -> None:
        """
        Args:
            control_port: the emulator's control channel, the same port `EspEmu` serves;
                `None` for a build without one
            app: `IdfApp` instance, whose partition table `erase_partition` reads
        """
    ...
    def erase_region(self, offset: int, size: int) -> None:
        ...
        send_control_command(self.control_port, f'erase-region {offset:#x} {size:#x}')
    ...
    def write_flash_no_enc(self, offset: int, file_path: str) -> None:
        ...
        send_control_command(self.control_port, f'write-region {offset:#x} {file_path}')
    ...
    def __getattr__(self, name: str) -> t.Any:
        raise NotImplementedError(
            f'esp-emu cannot do dut.serial.{name}() yet: it needs esptool '
            f'itself, driving the emulator in download mode with its UART on '
            f'a socket.'
        )
